...
-(void)setImageDeselected:(NSString *)deselectedName halfSelected:(NSString *)halfSelectedName fullSelected:(NSString *)fullSelectedName andDelegate:(id<RatingBarDelegate>)delegate{
    ...
    if (width*5<frame.size.width) {
        [_s1 setFrame:CGRectMake(0,         0, width, height)];
        [_s2 setFrame:CGRectMake(width+(frame.size.width-5*width)/4,     0, width, height)];
        [_s3 setFrame:CGRectMake(2 * (width+(frame.size.width-5*width)/4), 0, width, height)];
        [_s4 setFrame:CGRectMake(3 * (width+(frame.size.width-5*width)/4), 0, width, height)];
        [_s5 setFrame:CGRectMake(4 * (width+(frame.size.width-5*width)/4), 0, width, height)];
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    int newRating = 0;
    if (self.frame.size.width>5*width) {
        newRating = (int) (point.x / (width+(self.frame.size.width-5*width)/4)) + 1;
    }else{
        newRating = (int) (point.x / width) + 1;
    }
    if (newRating > 5)
        return;
